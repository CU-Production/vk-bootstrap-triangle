...
struct RenderData {
    ...
    VkCommandPool cs_command_pool;
    std::vector<VkCommandBuffer> command_buffers;
    std::vector<VkCommandBuffer> cs_command_buffers;
    ...
    struct {
        VkImage              image[2];
        VmaAllocation        vma_allocation[2];
...
int get_queues(Init& init, RenderData& data) {
    ...
    auto gid = init.device.get_queue_index(vkb::QueueType::graphics);
    if (!gid.has_value()) {
        std::cout << "failed to get graphics queue index: " << gid.error().message() << "\n";
        return -1;
    }
    data.graphics_queue_index = gid.value();
    ...
    auto cid = init.device.get_queue_index(vkb::QueueType::compute);
    if (!cid.has_value()) {
        std::cout << "failed to get graphics queue index: " << cid.error().message() << "\n";
        return -1;
    }
    data.compute_queue_index = cid.value();
...
int create_command_pool(Init& init, RenderData& data) {
    ...
    if (init.disp.createCommandPool(&pool_info, nullptr, &data.cs_command_pool) != VK_SUCCESS) {
        std::cout << "failed to create command pool\n";
        return -1; // failed to create command pool
    }
    return 0;
...
int create_command_buffers(Init& init, RenderData& data) {
    ...
    data.cs_command_buffers.resize(data.swapchain_image_views.size());
    ...
    allocInfo.commandPool = data.cs_command_pool;
    allocInfo.commandBufferCount = (uint32_t)data.cs_command_buffers.size();

    if (init.disp.allocateCommandBuffers(&allocInfo, data.cs_command_buffers.data()) != VK_SUCCESS) {
        return -1; // failed to allocate cs command buffers;
    }
    return 0;
}
...
int create_cs_noise_img(Init& init, RenderData& data) {
    ...
    if (vmaCreateImage(init.allocator, &cs_img_img_info, &cs_img_vma_alloc_info, &data.cs_noise_img.image[0], &data.cs_noise_img.vma_allocation[0], nullptr) != VK_SUCCESS) {
        std::cout << "failed to create cs_noise_img.image\n";
        return -1;
    }
    if (vmaCreateImage(init.allocator, &cs_img_img_info, &cs_img_vma_alloc_info, &data.cs_noise_img.image[1], &data.cs_noise_img.vma_allocation[1], nullptr) != VK_SUCCESS) {
        std::cout << "failed to create cs_noise_img.image\n";
        return -1;
    }
    ...
    // staging buffer copy
    {
        ...
        undefToShaderReadOptBarrier.image = data.cs_noise_img.image[0];
        ...
        VkImageMemoryBarrier undefToShaderReadOptBarrier2{};
        undefToShaderReadOptBarrier2.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
        undefToShaderReadOptBarrier2.image = data.cs_noise_img.image[1];
        undefToShaderReadOptBarrier2.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        undefToShaderReadOptBarrier2.subresourceRange.baseMipLevel = 0;
        undefToShaderReadOptBarrier2.subresourceRange.levelCount = 1;
        undefToShaderReadOptBarrier2.subresourceRange.baseArrayLayer = 0;
        undefToShaderReadOptBarrier2.subresourceRange.layerCount = 1;
        undefToShaderReadOptBarrier2.srcAccessMask = VK_ACCESS_NONE;
        undefToShaderReadOptBarrier2.dstAccessMask = VK_ACCESS_NONE;
        undefToShaderReadOptBarrier2.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        undefToShaderReadOptBarrier2.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        VkImageMemoryBarrier image_memory_barriers[2] = {undefToShaderReadOptBarrier, undefToShaderReadOptBarrier2};
    ...
    image_view_info.image = data.cs_noise_img.image[0];
    ...
    if (init.disp.createImageView(&image_view_info, nullptr, &data.cs_noise_img.image_view[0]) != VK_SUCCESS) {
        std::cout << "failed to create dummy_img.image_view\n";
        return -1;
    }

    image_view_info.image = data.cs_noise_img.image[1];

    if (init.disp.createImageView(&image_view_info, nullptr, &data.cs_noise_img.image_view[1]) != VK_SUCCESS) {
        std::cout << "failed to create dummy_img.image_view\n";
        return -1;
    }
...
int draw_frame(Init& init, RenderData& data) {
    ...
    const int current_noise_img_idx = image_index % 2;
    const int last_noise_img_idx    = (image_index - 1) % 2;

    {
        const int i = image_index;

        init.disp.resetCommandBuffer(data.command_buffers[i], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
        init.disp.resetCommandBuffer(data.cs_command_buffers[i], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
        ...
        if (init.disp.beginCommandBuffer(data.cs_command_buffers[i], &begin_info) != VK_SUCCESS) {
            return -1; // failed to begin recording command buffer
        }
        ...
        // dispatch noise compute shader
        {
            ...
            init.disp.cmdBeginDebugUtilsLabelEXT(data.cs_command_buffers[i], &debug_utils_label);

            // SRV -> UAV
            {
                VkImageMemoryBarrier ShaderReadOptToUndefBarrier{};
                ShaderReadOptToUndefBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
                ShaderReadOptToUndefBarrier.image = data.cs_noise_img.image[current_noise_img_idx];
                ...
                // ShaderReadOptToUndefBarrier.srcQueueFamilyIndex = data.graphics_queue_index;
                // ShaderReadOptToUndefBarrier.dstQueueFamilyIndex = data.compute_queue_index;

                init.disp.cmdPipelineBarrier(data.cs_command_buffers[i],
                    VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
            ...
            init.disp.cmdBindPipeline(data.cs_command_buffers[i], VK_PIPELINE_BIND_POINT_COMPUTE, data.compute_pipeline);

            ComputeShaderPushConstants constants{};
            constants.time = glfwGetTime();

            init.disp.cmdPushConstants(data.cs_command_buffers[i], data.compute_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(ComputeShaderPushConstants), &constants);

            VkDescriptorImageInfo descriptor_image_info{};
            descriptor_image_info.sampler = data.cs_noise_img.sampler;
            descriptor_image_info.imageView = data.cs_noise_img.image_view[current_noise_img_idx];
            ...
            init.disp.cmdPushDescriptorSetKHR(data.cs_command_buffers[i], VK_PIPELINE_BIND_POINT_COMPUTE, data.compute_pipeline_layout, 0, 1, &write_descriptor_set);

            for (int dispIdx = 0; dispIdx < 100; dispIdx++)
                init.disp.cmdDispatch(data.cs_command_buffers[i], 512 / 8, 512 / 8, 1);

            // UAV -> SRV
            {
                VkImageMemoryBarrier undefToShaderReadOptBarrier{};
                undefToShaderReadOptBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
                undefToShaderReadOptBarrier.image = data.cs_noise_img.image[current_noise_img_idx];
                undefToShaderReadOptBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
                undefToShaderReadOptBarrier.subresourceRange.baseMipLevel = 0;
                undefToShaderReadOptBarrier.subresourceRange.levelCount = 1;
                undefToShaderReadOptBarrier.subresourceRange.baseArrayLayer = 0;
            ...
            init.disp.cmdEndDebugUtilsLabelEXT(data.cs_command_buffers[i]);
        }

        if (init.disp.endCommandBuffer(data.cs_command_buffers[i]) != VK_SUCCESS) {
        ...
        VkDescriptorImageInfo descriptor_image_info{};
        descriptor_image_info.sampler = data.cs_noise_img.sampler;
        descriptor_image_info.imageView = data.cs_noise_img.image_view[last_noise_img_idx];
        ...
        for (int drawIdx = 0; drawIdx < 500; drawIdx++)
    ...
    {
        VkSubmitInfo cs_submitInfo = {};
        cs_submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
        cs_submitInfo.waitSemaphoreCount = 0;
        cs_submitInfo.pWaitDstStageMask = VK_NULL_HANDLE;
        cs_submitInfo.commandBufferCount = 1;
        cs_submitInfo.pCommandBuffers = &data.cs_command_buffers[image_index];
        cs_submitInfo.signalSemaphoreCount = 0;
        if (init.disp.queueSubmit(data.compute_queue, 1, &cs_submitInfo, VK_NULL_HANDLE) != VK_SUCCESS) {
...
void cleanup(Init& init, RenderData& data) {
    vmaDestroyImage(init.allocator, data.cs_noise_img.image[0], data.cs_noise_img.vma_allocation[0]);
    vmaDestroyImage(init.allocator, data.cs_noise_img.image[1], data.cs_noise_img.vma_allocation[1]);
    init.disp.destroyImageView(data.cs_noise_img.image_view[0], nullptr);
    init.disp.destroyImageView(data.cs_noise_img.image_view[1], nullptr);
    ...
    init.disp.destroyCommandPool(data.command_pool, nullptr);
    init.disp.destroyCommandPool(data.cs_command_pool, nullptr);
