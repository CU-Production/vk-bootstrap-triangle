...
struct RenderData {
    ...
    VkPipelineLayout pipeline_layout; // push constant still need it
...
int create_graphics_pipeline_layout(Init& init, RenderData& data) {
...
int create_shader_objects(Init& init, RenderData& data) {

    VkShaderCreateInfoEXT shader_create_infos[2]{};

    // With VK_EXT_shader_object we can generate an implementation dependent binary file
    bool binaryShadersLoaded = false;

    if (fileExists("shaders/triangle.vert.bin") && fileExists("shaders/triangle.frag.bin")) {
        auto vert_code = readFile("shaders/triangle.vert.bin");
        auto frag_code = readFile("shaders/triangle.frag.bin");
        ...
        shader_create_infos[0].nextStage = VK_SHADER_STAGE_FRAGMENT_BIT;
        shader_create_infos[0].codeType = VK_SHADER_CODE_TYPE_BINARY_EXT;
        shader_create_infos[0].pCode = vert_code.data();
        shader_create_infos[0].codeSize = vert_code.size();
        shader_create_infos[0].pName = "main";
        shader_create_infos[0].setLayoutCount = 0;
        shader_create_infos[0].pSetLayouts = VK_NULL_HANDLE;
        shader_create_infos[0].pushConstantRangeCount = 1;
        shader_create_infos[0].pPushConstantRanges = &vs_push_constant_range;
        ...
        shader_create_infos[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
        shader_create_infos[1].nextStage = 0;
        shader_create_infos[1].codeType = VK_SHADER_CODE_TYPE_BINARY_EXT;
        shader_create_infos[1].pCode = frag_code.data();
        shader_create_infos[1].codeSize = frag_code.size();
        shader_create_infos[1].pName = "main";
        shader_create_infos[1].setLayoutCount = 0;
        shader_create_infos[1].pSetLayouts = VK_NULL_HANDLE;
        shader_create_infos[1].pushConstantRangeCount = 1;
        shader_create_infos[1].pPushConstantRanges = &vs_push_constant_range;

        VkResult result = init.disp.createShadersEXT(2, shader_create_infos, nullptr, data.shaders);
        if (result == VK_SUCCESS) {
            binaryShadersLoaded = true;
        } else {
            std::cout << "Could not load binary shader files, loading SPIR - V instead\n";
        }
    }
    else {
        ...
        VkResult result = init.disp.createShadersEXT(2, shader_create_infos, nullptr, data.shaders);
        if (result != VK_SUCCESS) {
            std::cout << "Could not load spirv shader files\n";
            return -1;
        }

        // Store the binary shader files so we can try to load them at the next start
        size_t dataSize = 0;
        char* codebin_data = nullptr;
        std::fstream is;

        init.disp.getShaderBinaryDataEXT(data.shaders[0], &dataSize, nullptr);
        codebin_data = new char[dataSize];
        init.disp.getShaderBinaryDataEXT(data.shaders[0], &dataSize, codebin_data);
        is.open("shaders/phong.vert.bin", std::ios::binary | std::ios::out);
        is.write(codebin_data, dataSize);
        is.close();
        delete[] codebin_data;

        init.disp.getShaderBinaryDataEXT(data.shaders[1], &dataSize, nullptr);
        codebin_data = new char[dataSize];
        init.disp.getShaderBinaryDataEXT(data.shaders[1], &dataSize, codebin_data);
        is.open("shaders/phong.frag.bin", std::ios::binary | std::ios::out);
        is.write(codebin_data, dataSize);
        is.close();
        delete[] codebin_data;
    }
...
int draw_frame(Init& init, RenderData& data) {
...

    {
        ...
        init.disp.cmdSetViewportWithCountEXT(data.command_buffers[i], 1, &viewport);
        init.disp.cmdSetScissorWithCountEXT(data.command_buffers[i], 1, &scissor);
        // init.disp.cmdSetCullMode(data.command_buffers[i], VK_CULL_MODE_BACK_BIT);
        init.disp.cmdSetCullMode(data.command_buffers[i], VK_CULL_MODE_NONE);
        init.disp.cmdSetFrontFace(data.command_buffers[i], VK_FRONT_FACE_COUNTER_CLOCKWISE);
        ...
        init.disp.cmdSetPrimitiveRestartEnable(data.command_buffers[i], VK_FALSE);

        const uint32_t sampleMask = 0xFF;
        init.disp.cmdSetSampleMaskEXT(data.command_buffers[i], VK_SAMPLE_COUNT_1_BIT, &sampleMask);
        ...
        init.disp.cmdSetColorBlendEnableEXT(data.command_buffers[i], 0, 1, &colorBlendEnables);
        init.disp.cmdSetColorWriteMaskEXT(data.command_buffers[i], 0, 1, &colorBlendComponentFlags);
        ...
        std::vector<VkVertexInputAttributeDescription2EXT> vertexAttributes = {
            ...
            { VK_STRUCTURE_TYPE_VERTEX_INPUT_ATTRIBUTE_DESCRIPTION_2_EXT, nullptr, 1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, color) }
        };

        init.disp.cmdSetVertexInputEXT(data.command_buffers[i], 1, &vertexInputBinding, 2, vertexAttributes.data());
        ...
        init.disp.cmdBindShadersEXT(data.command_buffers[i], 2, stages, data.shaders);
...
int main() {
    ...
    if (0 != create_graphics_pipeline_layout(init, render_data)) return -1;
    if (0 != create_shader_objects(init, render_data)) return -1;
