...
int create_sync_objects(Init& init, RenderData& data) {
    ...
    data.finished_semaphores.resize(MAX_FRAMES_IN_FLIGHT);
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (init.disp.createSemaphore(&semaphore_info, nullptr, &data.available_semaphores[i]) != VK_SUCCESS ||
            init.disp.createSemaphore(&semaphore_info, nullptr, &data.finished_semaphores[i]) != VK_SUCCESS) {
    ...
    if (init.disp.createSemaphore(&semaphore_info, nullptr, &data.timeline_semaphore) != VK_SUCCESS) {
        std::cout << "failed to create data.timeline_semaphore\n";
        return -1; // failed to create synchronization objects for a frame
    }
...
int draw_frame(Init& init, RenderData& data) {
    if (data.number_of_frame >= MAX_FRAMES_IN_FLIGHT) {
        const uint64_t timeline_value = data.number_of_frame - (MAX_FRAMES_IN_FLIGHT - 1);

        VkSemaphoreWaitInfo semaphore_wait_info{};
        semaphore_wait_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_WAIT_INFO;
        semaphore_wait_info.semaphoreCount = 1;
        semaphore_wait_info.pSemaphores = &data.timeline_semaphore;
        semaphore_wait_info.pValues = &timeline_value;

        if (init.disp.waitSemaphores(&semaphore_wait_info, UINT64_MAX) != VK_SUCCESS) {
            std::cout << "failed to wait timeline semaphore.\n";
            return -1;
        };
    }
    ...
    wait_semaphores[0].semaphore = data.available_semaphores[data.current_frame];
    wait_semaphores[0].value = 0;
    wait_semaphores[0].stageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT;
    wait_semaphores[0].deviceIndex = 0;
    wait_semaphores[1].sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO;
    wait_semaphores[1].semaphore = data.timeline_semaphore;
    wait_semaphores[1].value = data.number_of_frame >= MAX_FRAMES_IN_FLIGHT ? data.number_of_frame - (MAX_FRAMES_IN_FLIGHT - 1) : 0;
    wait_semaphores[1].stageMask = VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT;
    wait_semaphores[1].deviceIndex = 0;

    std::array<VkSemaphoreSubmitInfo, 2> signal_semaphores{};
    signal_semaphores[0].sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO;
    signal_semaphores[0].semaphore = data.finished_semaphores[data.current_frame];
    signal_semaphores[0].value = 0;
    signal_semaphores[0].stageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT;
    signal_semaphores[0].deviceIndex = 0;
    signal_semaphores[1].sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO;
    signal_semaphores[1].semaphore = data.timeline_semaphore;
    signal_semaphores[1].value = data.number_of_frame + 1;
    signal_semaphores[1].stageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT;
    signal_semaphores[1].deviceIndex = 0;

    VkCommandBufferSubmitInfo command_buffer_info{};
    command_buffer_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_SUBMIT_INFO_KHR;
    command_buffer_info.commandBuffer = data.command_buffers[image_index];
    ...
    if (init.disp.queueSubmit2(data.graphics_queue, 1, &submit_info, VK_NULL_HANDLE) != VK_SUCCESS) {
    ...
    present_info.pWaitSemaphores = &data.finished_semaphores[data.current_frame];
    ...
    data.last_frame = data.current_frame;
...
void cleanup(Init& init, RenderData& data) {
    ...
    init.disp.destroySemaphore(data.timeline_semaphore, nullptr);
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        init.disp.destroySemaphore(data.finished_semaphores[i], nullptr);
