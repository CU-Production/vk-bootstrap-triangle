...
int device_initialization(Init& init) {
    ...
    auto phys_device_ret = phys_device_selector
        ...
        .add_required_extension_features(mesh_shader_feature)
...
int create_graphics_pipeline(Init& init, RenderData& data) {
    auto task_code = readFile("shaders/triangle.task.spv");
    auto mesh_code = readFile("shaders/triangle.mesh.spv");
    auto frag_code = readFile("shaders/triangle.frag.spv");

    VkShaderModule task_module = createShaderModule(init, task_code);
    VkShaderModule mesh_module = createShaderModule(init, mesh_code);
    VkShaderModule frag_module = createShaderModule(init, frag_code);
    if (task_module == VK_NULL_HANDLE || mesh_module == VK_NULL_HANDLE || frag_module == VK_NULL_HANDLE) {
        std::cout << "failed to create shader module\n";
        return -1; // failed to create shader modules
    }

    VkPipelineShaderStageCreateInfo task_stage_info = {};
    ...
    task_stage_info.module = task_module;
    task_stage_info.pName = "main";

    VkPipelineShaderStageCreateInfo mesh_stage_info = {};
    ...
    mesh_stage_info.module = mesh_module;
    ...
    VkPipelineShaderStageCreateInfo shader_stages[] = { task_stage_info, mesh_stage_info, frag_stage_info };
    ...
    vs_push_constant_range.stageFlags = VK_SHADER_STAGE_MESH_BIT_EXT;
    ...
    init.disp.destroyShaderModule(mesh_module, nullptr);
    init.disp.destroyShaderModule(task_module, nullptr);
    return 0;
}
...
int draw_frame(Init& init, RenderData& data) {
...

    {
        ...
        init.disp.cmdPushConstants(data.command_buffers[i], data.pipeline_layout, VK_SHADER_STAGE_MESH_BIT_EXT, 0, sizeof(VertexShaderPushConstants), &constants);

        // init.disp.cmdDraw(data.command_buffers[i], 3, 1, 0, 0);
        init.disp.cmdDrawMeshTasksEXT(data.command_buffers[i], 1, 1, 1);
