...
int device_initialization(Init& init) {
    init.window = create_window_glfw("Ozz-CS Skinning Demo", true);
...
int create_vertex_buffer(Init& init, RenderData& data) {
    ...
    data.vertex_buffer_descriptor_buffer_info.buffer = data.vertex_buffer;
    data.vertex_buffer_descriptor_buffer_info.offset = 0;
    data.vertex_buffer_descriptor_buffer_info.range = buffer_size;
...
int create_index_buffer(Init& init, RenderData& data) {
    ...
    index_buffer_info.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    ...
    data.index_buffer_descriptor_buffer_info.buffer = data.index_buffer;
    data.index_buffer_descriptor_buffer_info.offset = 0;
    data.index_buffer_descriptor_buffer_info.range = buffer_size;
...
int create_ssbo(Init& init, RenderData& data) {
    ...
    if (vmaCreateBuffer(init.allocator, &ssbo_info, &index_buffer_alloc_info, &data.shader_storage_buffer, &data.shader_storage_buffer_allocation, nullptr) != VK_SUCCESS) {
        std::cout << "failed to create ssbo\n";
        return -1; // failed to create vertex buffer
    }

    void* mapped_data;
    vmaMapMemory(init.allocator, data.shader_storage_buffer_allocation, &mapped_data);
    memcpy(mapped_data, data.indices.data(), ssbo_info.size);
    vmaUnmapMemory(init.allocator, data.shader_storage_buffer_allocation);

    data.shader_storage_buffer_descriptor_buffer_info.buffer = data.shader_storage_buffer;
    data.shader_storage_buffer_descriptor_buffer_info.offset = 0;
    data.shader_storage_buffer_descriptor_buffer_info.range = buffer_size;
